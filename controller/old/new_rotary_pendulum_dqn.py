...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.model.load_weights("./save/rotary_pendulum_dqn_good.h5")
...
if __name__ == "__main__":
    ...
    for episode in range(episode_start_num, EPISODES):
        ...
        while not done:
            ...
            #         cnt = max_cnt
            #     else:
            #         if state[0][0] < last_state:
            #             action_index = 0
            #             cnt -= 1
            #         elif state[0][0] > last_state:
            #             action_index = 2
            #             cnt -= 1
            #         elif cnt < max_cnt and state[0][0] == last_state:
            ...
            # sin_pen_rad = math.sin(next_state[0])
            # cos_pen_rad = math.cos(next_state[0])
            # next_state = [sin_pen_rad, cos_pen_rad, next_state[1], next_state[3]]
            ...
            if not(done and step == 1):
                ...
                agent.append_sample(flat_history, action_index, reward, flat_next_history, done)
