...
through the microcontroller interface panel.
...
Upon successfully building and uploading the code, the QUBE Servo 2 Inertia Disc should
oscillate between two positions roughly 90 deg apart.
...
byte tach0Byte[3] f= {0,0,0};        // arm tachometer
...
void loop() {
  ...
  if (currentMicros - previousMicros >= sampleTime) {
    ...
    /*Start of Custom Code*/
        ...
        switchTime = switchTime + (1.0/sampleTime);

        if (switchTime > (setpointP/2.0))
        {
          if (setPoint == -setpointAngle)
          {
            ...
            switchTime = 0.0;
            LEDRed = 0;
            LEDGreen = 999;  // turn on the green LEDs, maximum intensity
            LEDBlue = 0;
          }
          else
          {
            setPoint = -setpointAngle;
            switchTime = 0.0;
            LEDRed = 999;  // turn on the red LEDs, maximum intensity
            LEDGreen = 0;
            LEDBlue = 0;
          }
        }
      ...
      float kp_theta = 2.0;
      float kd_theta = -0.05;
      motorVoltage = (-theta_n * kp_theta) + (theta_dot * kd_theta);
      
      // set the saturation limit to +/- 10V
      if (motorVoltage > 10.0) {
        motorVoltage = 10.0;
      }
      else if (motorVoltage < -10.0) {
        motorVoltage = -10.0;
      }
